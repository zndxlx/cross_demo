...
ANDROID_BUILD_CMD = 'cmake "%s" %s -DANDROID_ABI="%s" -DCMAKE_BUILD_TYPE=Release -DCMAKE_TOOLCHAIN_FILE=%s/build/cmake/android.toolchain.cmake -DANDROID_TOOLCHAIN=gcc -DANDROID_NDK=%s -DANDROID_PLATFORM=android-14 -DANDROID_STL="c++_shared" && cmake --build . --target install %s --config Release -- -j8'
ANDROID_SYMBOL_PATH = 'libraries/mars_android_sdk/obj/local/'  # strip前的库
ANDROID_LIBS_PATH = 'libraries/mars_android_sdk/libs/'  # strip后的库
...
def build_android(incremental, arch):
    ...
    os.mkdir(lib_path)
    print("1111111 ANDROID_LIBS_INSTALL_PATH=%s\n" % (ANDROID_LIBS_INSTALL_PATH))
    for f in glob.glob(ANDROID_LIBS_INSTALL_PATH+"Android.out/" + "*"):
